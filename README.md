...
项目目前是一个原生 Apple 双端 0.1 Beta：

- `TypeCarrieriOS`：iPhone 输入和发送端。
- `TypeCarrierMac`：macOS 菜单栏接收端，收到文本后自动粘贴到当前输入焦点。
- `TypeCarrierCore`：共享 payload、连接状态和 Multipeer 传输逻辑。

0.1 使用 Apple 的 Multipeer Connectivity 在局域网内通信，不需要账号，也不经过服务器。当前工程目标系统版本为 iOS 26.0 和 macOS 26.0。
...
- 0.1 不做云同步、二维码配对、配对码、AI、语音识别、Android、Windows 或多设备切换。

## 下载与发布

0.1 Beta 通过 GitHub prerelease 发布：

- iOS 端只提供源码，不提供 GitHub 可安装包。
- macOS zip 是 Apple Development / Personal Team 签名的测试包，适合技术预览和小范围试用。
- macOS 测试包不是 Developer ID notarized 正式包，Gatekeeper 可能拦截。
- TestFlight、App Store、Developer ID notarized macOS 包需要付费 Apple Developer Program，后续正式 Beta 或正式版本再考虑。
