...
当前 GitHub Release 仍按 beta / 侧载定位处理：

- Android 和 macOS 包通过 GitHub Release 提供，不是应用商店分发。
- macOS 测试包可能仍是 Apple Development / Personal Team 签名，不一定是 Developer ID notarized 正式包，Gatekeeper 可能拦截。
- iOS 不在 GitHub Release 提供可安装包；正式获取方式以 App Store 为准。
